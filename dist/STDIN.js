...
const fs = require("fs");
const judgeHands_1 = require("./judgeHands");
...
const handleData = (data) => {
    ...
    data.map((data) => {
        ...
        let handOne = { values: [], suits: [] };
        let handTwo = { values: [], suits: [] };
        let handOneTest = "";
        ...
        splitedData.forEach((handData, index) => {
            ...
            if (index < 5) {
                ...
                handOne.values.push(parseInt(handData[0])
                    ? +handData[0]
                ...
                if (index === 4)
                    playerOne = new Player_1.PlayerClass(handOne, 1).run();
            }
            else {
                handTwo.values.push(parseInt(handData[0])
                    ? +handData[0]
                    : //traslating alphabetics to numbers to nake comperisions easier
                        handData[0] === "T"
                            ? 10
            ...
            if (index === 9) {
                playerTwo = new Player_1.PlayerClass(handTwo, 2).run();
                const { playerOneWins, playerTwoWins } = judgeHands_1.judgeHandsAndFindTheWinner(playerOne, playerTwo);
...
console.log(`Player One :  ${judgement.playerOneWins} Hands,`, "\n", `Player Two :  ${judgement.playerTwoWins} Hands,`);
//Creating the STDOUT.text file containing the programm results.
fs.writeFile("STDOUT.txt", `Player One :  ${judgement.playerOneWins} Hands \n Player Two :  ${judgement.playerTwoWins} Hands`, (err) => {
    err
        ? console.log(err)
        : console.log("STDOUT Ready to read. You can find this file in root folder of the project. Thank you for your time.");
});
// });
